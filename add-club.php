...
<html>
    ...
    <body>
        ...
        <div class="login-header">
            <div class="address"><a href="index.php"> Home </a> &nbsp > &nbsp <a href="admin.php">Admin Login </a> &nbsp > &nbsp <a href="adpanel.php">Panel</a> &nbsp > &nbsp Add Club  </div>
        ...
        <div style="color : white;">
		    ...
		    $query="select MAX(no) from clubpart";
		   ...
		   $row=mysqli_fetch_array($result);
		    $a=$row[0];
                ...
                $a++;
                $name=$_POST['b'];
		$query="select * from clubpart where clubname='$name' ";
		 $result=mysqli_query($dbc,$query);
		   while($row=mysqli_fetch_array($result))
		   {
		     echo '<script>alert("Club is already added");</script>';
		     return false;
		   }
		     mysqli_close($dbc);
		...
		move_uploaded_file($_FILES['img']['tmp_name'],$target);
                ...
                $sql="INSERT INTO clubpart(no,clubname,image,Descrip) values('".$a."','".$name."','".$upload."','".$descrip."')";
                if(!$mysqli->query($sql))
                ...
                $mysqli->close();
                            ...
                            fclose($myfile);
                             $dbc=mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME)or die("unable to connect to the database");
                            $query="update clubpart set Post=NOW() where no='$a' ";
                            ...
                            mysqli_query($dbc,$query) or die('Error in querying');
                            mysqli_close($dbc);
                           echo '<script>alert("Club is created Successfully");location.href="adpanel.php";</script>';
                        }
                        ?>
        </div>
	     <div class="news-header">
                    <form  action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
                        <table cellpadding="20" cellspacing="10" align="center" class="news-table">
                             <tr>
                                <td colspan="2" class="update-form-heading">Add Club</td>
                             </tr>
                             <tr>
                                 <td style="width : 50% ;">Club Name*</td>
				 <td><input type='text' id="text" placeholder="Enter Club Name"   name='b' onblur="name_validator()" required/></td>
                             </tr>
                              <tr>
			       ...
			       <tr>
                        ...
                        <div class="center">
                            ...
                            <input type="submit" id="search-op"  value="Add Club" name="add">
                        </div>
                    </form>
                     <hr class="divider1">
