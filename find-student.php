<html>
    ...
    <body>
	    ...
	    <?php require_once('connectvars.php'); ?>
            ...
            {   $out=true;
                if(!isset($_POST['change']))
               {
                ...
                if($out)
                {
                   ...
                   $query="delete from logins where uid in ( $ids )";
                   mysqli_query($dbc,$query) or die('Error in querying');
                   $query="update student set del='1'  where sno in ( $ids )";
                   mysqli_query($dbc,$query) or die('Error in querying');
                   echo '<script>alert("Student Records are deleted successfully");location.href="adpanel.php";</script>';
                }
            }
            ?>
			<header>
            ...
            if($year=='all')
            {
            $query="select * from student where course='$course' and branch='$branch' " ;
            }
            else
            {
                 if(isset($_POST['sem']))
            ...
            $query="select * from student where course='$course' and branch='$branch' and year='$year' and sem='$sem' " ;
            }
               ...
               while($row=mysqli_fetch_array($result))
               {
                ...
                <tr>
                  <?php
                    ...
                    <?php if($del=='0')
                    {?>
                    <td><input type="checkbox" name="change[]" value="<?php echo $sno ;?>"></td>
                    <?php }
                    else
                    {?>
                    <td>Deleted</td>
