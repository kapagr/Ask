	...
	<?php require_once('connectvars.php'); ?>
         <?php
			    ...
			    $dbc=mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME)or die("unable to connect to the database");
                            $query="select univrollno from student where univrollno='$roll' ";
			    $result=mysqli_query($dbc,$query) or die(mysqli_error());
			   if($row=mysqli_fetch_array($result)==0)
			       {
					 echo '<script>alert("Please Check Your University Roll No");</script>';
					 echo '<script>location.href="search-std.php";</script>';
			        }
				else
				{
					$getreq="search-student.php?rollno=$roll";
      ...
      ?>
        ...
        <div style="color : brown ; font-size : 1em;font-family : 'Mistral';margin-top : 4% ; ">Search Student :)</div>
